#!/bin/bash
# Benchmark script for Parquet File Reader
# Usage: bash benchmark/run.sh [input_parquet] [repeats]
...
RESULT_FILE="$RESULTS_DIR/parquet-file-reader_$(date +%Y%m%d_%H%M%S).txt"
...
if [ ! -f "$PROJECT_DIR/$INPUT_FILE" ] && [ ! -f "$INPUT_FILE" ]; then
    python3 "$PROJECT_DIR/test-data/generate.py"
fi

INPUT_ABS="$(cd "$PROJECT_DIR" && realpath "$INPUT_FILE")"
INPUT_DIR="$(dirname "$INPUT_ABS")"
INPUT_BASENAME="$(basename "$INPUT_ABS")"

echo "╔══════════════════════════════════════════╗"
echo "║      Parquet File Reader Benchmark       ║"
echo "╚══════════════════════════════════════════╝"
...
build_image "pfr-go"   "$PROJECT_DIR/go"
build_image "pfr-rust" "$PROJECT_DIR/rust"
build_image "pfr-zig"  "$PROJECT_DIR/zig"
echo ""
...
run_benchmark() {
    ...
    for i in $(seq 1 $RUNS); do
        ...
        output=$(docker run --rm -v "$INPUT_DIR":/data:ro "$image" "/data/$INPUT_BASENAME" "$REPEATS" 2>&1)
        exit_code=$?
...
run_benchmark "Go"   "pfr-go"
run_benchmark "Rust" "pfr-rust"
run_benchmark "Zig"  "pfr-zig"
...
echo "── Binary Size ───────────────────────────────"
printf "  Go  : %s\n" "$(get_binary_size pfr-go   /usr/local/bin/parquet-file-reader)"
printf "  Rust: %s\n" "$(get_binary_size pfr-rust /usr/local/bin/parquet-file-reader)"
printf "  Zig : %s\n" "$(get_binary_size pfr-zig  /usr/local/bin/parquet-file-reader)"
echo ""
