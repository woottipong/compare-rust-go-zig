#!/bin/bash
# Benchmark script for JPEG Thumbnail Pipeline
# Usage: bash benchmark/run.sh [input_jpg] [width] [height] [repeats]
...
RESULT_FILE="$RESULTS_DIR/jpeg-thumbnail-pipeline_$(date +%Y%m%d_%H%M%S).txt"
...
echo "╔══════════════════════════════════════════╗"
echo "║    JPEG Thumbnail Pipeline Benchmark     ║"
echo "╚══════════════════════════════════════════╝"
...
build_image "jtp-go"   "$PROJECT_DIR/go"
build_image "jtp-rust" "$PROJECT_DIR/rust"
build_image "jtp-zig"  "$PROJECT_DIR/zig"
echo ""
...
run_benchmark() {
    ...
    for i in $(seq 1 $RUNS); do
        ...
        output=$(docker run --rm -v "$INPUT_DIR":/data:ro "$image" "/data/$INPUT_FILE" "/tmp/out.jpg" "$WIDTH" "$HEIGHT" "$REPEATS" 2>&1)
        exit_code=$?
...
echo "── Binary Size ───────────────────────────────"
printf "  Go  : %s\n" "$(get_binary_size jtp-go   /usr/local/bin/jpeg-thumbnail)"
printf "  Rust: %s\n" "$(get_binary_size jtp-rust /usr/local/bin/jpeg-thumbnail)"
printf "  Zig : %s\n" "$(get_binary_size jtp-zig  /usr/local/bin/jpeg-thumbnail)"
echo ""
