#!/bin/bash
# Benchmark script for PNG Encoder from Scratch
# Usage: bash benchmark/run.sh [input_ppm] [repeats]
...
RESULT_FILE="$RESULTS_DIR/png-encoder-from-scratch_$(date +%Y%m%d_%H%M%S).txt"
...
if [ ! -f "$PROJECT_DIR/$INPUT_PPM" ] && [ ! -f "$INPUT_PPM" ]; then
    echo "Input not found. Generating test-data/sample.ppm"
    python3 "$PROJECT_DIR/test-data/generate.py"
fi

INPUT_ABS="$(cd "$PROJECT_DIR" && realpath "$INPUT_PPM")"
INPUT_DIR="$(dirname "$INPUT_ABS")"
INPUT_FILE="$(basename "$INPUT_ABS")"

echo "╔══════════════════════════════════════════╗"
echo "║   PNG Encoder From Scratch Benchmark     ║"
echo "╚══════════════════════════════════════════╝"
echo "  Input    : $INPUT_PPM"
...
build_image "pge-go"   "$PROJECT_DIR/go"
build_image "pge-rust" "$PROJECT_DIR/rust"
build_image "pge-zig"  "$PROJECT_DIR/zig"
echo ""
...
run_benchmark() {
    ...
    for i in $(seq 1 $RUNS); do
        ...
        output=$(docker run --rm -v "$INPUT_DIR":/data:ro "$image" "/data/$INPUT_FILE" "/tmp/output.png" "$REPEATS" 2>&1)
        exit_code=$?
...
run_benchmark "Go"   "pge-go"
run_benchmark "Rust" "pge-rust"
run_benchmark "Zig"  "pge-zig"
...
echo "── Binary Size ───────────────────────────────"
printf "  Go  : %s\n" "$(get_binary_size pge-go   /usr/local/bin/png-encoder)"
printf "  Rust: %s\n" "$(get_binary_size pge-rust /usr/local/bin/png-encoder)"
printf "  Zig : %s\n" "$(get_binary_size pge-zig  /usr/local/bin/png-encoder)"
echo ""
