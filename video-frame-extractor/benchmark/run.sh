...
echo "[Go]"
(unset GOROOT && cd "$PROJECT_DIR/go" && go build -o ../bin/extractor-go . 2>&1) \
    && echo "  ✓ bin/extractor-go" || echo "  ✗ build failed"
...
(cd "$PROJECT_DIR/rust" && \
  ...
  cargo build --release 2>&1 | grep -E "^error|Finished|Compiling video") \
    && echo "  ✓ rust/target/release/video-frame-extractor" || echo "  ✗ build failed"

echo "[Zig]"
(cd "$PROJECT_DIR/zig" && zig build -Doptimize=ReleaseFast 2>&1) \
    && echo "  ✓ zig/zig-out/bin/video-frame-extractor" || echo "  ✗ build failed"
...
run_benchmark() {
    ...
    for i in $(seq 1 $RUNS); do
        local output="/tmp/bench_${name}_$$.ppm"
        local start end elapsed

        start=$(date +%s%N)
        if /usr/bin/time -l $cmd "$INPUT_VIDEO" "$TIMESTAMP" "$output" >/dev/null 2>/tmp/bench_time_$$.txt; then
            end=$(date +%s%N)
...
# ─── Run benchmarks ───────────────────────────────────────────────────────────
[ -f "$PROJECT_DIR/bin/extractor-go" ] && \
    run_benchmark "Go" "$PROJECT_DIR/bin/extractor-go"

[ -f "$PROJECT_DIR/rust/target/release/video-frame-extractor" ] && \
    run_benchmark "Rust" "$PROJECT_DIR/rust/target/release/video-frame-extractor"

[ -f "$PROJECT_DIR/zig/zig-out/bin/video-frame-extractor" ] && \
    run_benchmark "Zig" "$PROJECT_DIR/zig/zig-out/bin/video-frame-extractor"

# ─── Binary Size ──────────────────────────────────────────────────────────────
echo "── Binary Size ───────────────────────────────"
[ -f "$PROJECT_DIR/bin/extractor-go" ] && \
    ls -lh "$PROJECT_DIR/bin/extractor-go" | awk '{printf "  Go  : %s\n", $5}'
[ -f "$PROJECT_DIR/rust/target/release/video-frame-extractor" ] && \
    ls -lh "$PROJECT_DIR/rust/target/release/video-frame-extractor" | awk '{printf "  Rust: %s\n", $5}'
[ -f "$PROJECT_DIR/zig/zig-out/bin/video-frame-extractor" ] && \
    ls -lh "$PROJECT_DIR/zig/zig-out/bin/video-frame-extractor" | awk '{printf "  Zig : %s\n", $5}'
