...
use crate::protocol::{Message, MSG_CHAT, MSG_JOIN, MSG_LEAVE, MSG_PING, MSG_PONG, PING_INTERVAL_SEC, RATE_LIMIT_MSG_PER_SEC};
...
pub async fn handle_connection(
            ...
            tokio::select! {
                ...
                _ = ping_ticker.tick() => {
                    let ts = std::time::SystemTime::now()
                        ...
                        .unwrap_or_default()
                        .as_millis() as i64;
                    let ping = serde_json::to_string(&Message {
                        msg_type: MSG_PING.to_string(),
                        room: None, user: None,
                        text: None, ts: Some(ts),
                    }).unwrap_or_default();
                    if sink.send(WsMessage::Text(ping.into())).await.is_err() { break; }
                }
            }
        }
    });
    ...
    loop {
        ...
        let raw = match item {
            ...
            Ok(WsMessage::Pong(_)) => continue, // keepalive pong
            Ok(WsMessage::Close(_)) => break,
            Err(_) => break,
            _ => continue,
        };

        let msg: Message = match serde_json::from_str(raw.as_str()) {
            Ok(m) => m,
...
mod tests {
    ...
    fn test_rate_limit_drop() {
        ...
        assert_eq!(passed, TOKEN_MAX, "expected {TOKEN_MAX} to pass");
        assert_eq!(dropped, 20 - TOKEN_MAX, "expected {} to drop", 20 - TOKEN_MAX);
    }
    ...
    fn test_rate_limit_refill() {
        ...
        assert!(!limiter.allow(), "should be empty");

        // force 1s elapsed
        limiter.last_refill = Instant::now() - Duration::from_secs(1);
        assert!(limiter.allow(), "should refill after 1s");
    }
}
