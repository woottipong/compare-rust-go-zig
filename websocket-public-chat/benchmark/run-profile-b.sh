...
run_scenario() {
    ...
    local cpu_file="/tmp/wsc_${lang}_${scenario}_cpu"
    echo "0" > "$cpu_file"
    {
        peak_cpu=0
        while true; do
    ...
    printf '%s\n' "$tp"       >> "/tmp/wsc_${lang}_${scenario}_tp"
    printf '%s\n' "$p95"      >> "/tmp/wsc_${lang}_${scenario}_p95"
    printf '%s\n' "$peak_mem" >> "/tmp/wsc_${lang}_${scenario}_mem_peak"
    printf '%s\n' "$peak_cpu" >> "/tmp/wsc_${lang}_${scenario}_cpu_peak"
}
...
print_summary_table() {
    ...
    for lang in "${LANGUAGES[@]}"; do
        st=$(format_stat "/tmp/wsc_${lang}_steady_tp")
        bu=$(format_stat "/tmp/wsc_${lang}_burst_tp")
        sa=$(format_stat "/tmp/wsc_${lang}_saturation_tp")
        mem=$(format_stat "/tmp/wsc_${lang}_saturation_mem_peak")
        cpu=$(format_stat "/tmp/wsc_${lang}_saturation_cpu_peak")
...
rm -f /tmp/wsc_*_*_tp /tmp/wsc_*_*_p95 /tmp/wsc_*_*_mem_peak /tmp/wsc_*_*_cpu_peak
