...
RESULT_FILE="$SCRIPT_DIR/results/websocket_public_chat_${TIMESTAMP}.txt"
...
run_scenario() {
    ...
    # Write to temp files so we can read them for the summary table
...
echo "╔══════════════════════════════════╗"
echo "║  WebSocket Public Chat Benchmark ║"
echo "╚══════════════════════════════════╝"
echo "Date: $(date)"
echo
...
for lang in go rust zig; do
    ...
    ch=$(cat "/tmp/wsc_${lang}_churn_tp"  2>/dev/null || echo "N/A")
    printf "%-8s  %22s  %22s  %22s\n" "$lang" "$st" "$bu" "$ch"
done
rm -f /tmp/wsc_*_*_tp
