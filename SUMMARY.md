...
**1. หน่วยต่างกันตาม domain — ห้ามเทียบข้ามโปรเจกต์**
`items/s` ใน SQLite Query Engine หมายถึง B-tree rows ที่สแกนได้ต่อวินาที — ไม่เหมือนกับ `items/s` ใน Log Masker ที่หมายถึง bytes ต่อวินาที ผลทุกตัวในตารางเทียบกันได้เฉพาะ **ข้ามภาษาในโปรเจกต์เดียวกัน** เท่านั้น

**2. ผลต่าง < ~10% = "เท่ากันในทางปฏิบัติ"**
บน Docker ARM (Apple Silicon) แต่ละรอบมี variance ±5-8% จากการ schedule และ memory allocation ตัวอย่าง:
- DNS Resolver: Rust 6,155 vs Zig 5,492 items/s → ต่างกัน 12% → ชนะแบบ marginal
- QUIC Ping Client: Go/Rust/Zig ≈ 6,000-6,300 items/s → เท่ากัน (hardware bottleneck)

เฉพาะผลต่างระดับ **2× ขึ้นไป** ถือเป็น structural advantage จริง (เช่น TCP Scanner Rust 108K vs Go 664 items/s = 163×)

**3. กลุ่ม 1 ใช้ elapsed ms ไม่ใช่ throughput**
Video/HLS/Subtitle (โปรเจกต์ 1.1–1.3) รายงานเป็นมิลลิวินาทีรวมตั้งแต่ start จนจบ เพราะงาน ffmpeg pipeline ไม่ได้วัด ops/s แต่วัดว่า "เร็วแค่ไหน" ตัวเลขรวม Docker startup ~400-500ms อยู่ด้วย ดังนั้น `*` ในตารางหมายความว่า **เทียบข้ามภาษาในโปรเจกต์เดียวกันเท่านั้น** อย่านำไปเทียบกับโปรเจกต์อื่น

**4. ws_errors/s ใน WebSocket สะท้อนอะไรในทางปฏิบัติ?**
Go (GoFiber) ได้ 2.54 ws_errors/s ใน steady-soak 300s = ~762 errors รวม ใน 100 connections หรือประมาณ 0.25% ของ messages ที่ส่ง สำหรับ **public chat**: ยอมรับได้ สำหรับ **financial feed หรือ telemetry**: ไม่ยอมรับได้ Rust และ Zig ได้ 0.00 errors ตลอด 480s
